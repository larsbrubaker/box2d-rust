...
// Deserializers assign fields onto Default::default() sequentially because
// every read is side-effecting: the assignment order IS the wire order, and
...
pub(crate) fn r_vec2(r: &mut SnapReader) -> Vec2 {
    Vec2 {
        x: r.r_f32(),
        y: r.r_f32(),
    }
...
pub(crate) fn r_rot(r: &mut SnapReader) -> Rot {
    Rot {
        c: r.r_f32(),
        s: r.r_f32(),
    }
...
pub(crate) fn r_xf(r: &mut SnapReader) -> Transform {
    Transform {
        p: r_vec2(r),
        q: r_rot(r),
    }
...
pub(crate) fn w_softness(buf: &mut Vec<u8>, s: Softness) {
    rec_w_f32(buf, s.bias_rate);
    rec_w_f32(buf, s.mass_scale);
    rec_w_f32(buf, s.impulse_scale);
}

pub(crate) fn r_softness(r: &mut SnapReader) -> Softness {
    Softness {
        bias_rate: r.r_f32(),
        mass_scale: r.r_f32(),
...
pub(crate) fn w_mat22(buf: &mut Vec<u8>, m: Mat22) {
    rec_w_vec2(buf, m.cx);
    rec_w_vec2(buf, m.cy);
}

pub(crate) fn r_mat22(r: &mut SnapReader) -> Mat22 {
    Mat22 {
        cx: r_vec2(r),
        cy: r_vec2(r),
    }
