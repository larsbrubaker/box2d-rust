...
export interface SimPlayer {
  ...
  set_keyframe_policy(budgetMb: number, minInterval: number): void;
  keyframe_budget_mb(): number;
  keyframe_min_interval(): number;
  ...
  contact_count(): number;
  body_count(): number;
  frame_query_count(): number;
  set_selection(kind: number, bodyOrd: number, slot: number, query: number): void;
  selection(): Int32Array;
  pick_at(x: number, y: number): Int32Array;
  outline_json(): string;
  detail_text(): string;
  collect_draw(lowerX: number, lowerY: number, upperX: number, upperY: number): void;
  draw_polygons(): Float32Array;
